...
var models, peers = [], $ = function (sel, node, a) { return (a = [].slice.call( (node || document).querySelectorAll(sel) )).length > 1 ? a : a[0] };
...
$.merge($, {
  ...
  hash: function(s) {
    var hash = 0, i = s.length, hs;
    while (i--) hash = ((hash << 5) - hash) + s.charCodeAt(i) >>> 0;
    return Array(9 - (hs = hash.toString(16)).length).join("0") + hs
  },
  scrolling: {
    tid: null,
    id: [],
  ...
  markdown: function (ta, style) { //TODO: escape **//``
    ...
    for (var p = (newline === "" ? [r] : r.split(newline + newline)), i = 0; i < p.length; i++) {
      ...
      if (style === "body") p[i] = p[i].replace(/(.+)/g, "<p>$1</p>")
    }
    r = p.join(newline);
    return r
  },
  ...
  initdb: function (network) {
    return new Promise(function (resolve) {
      var name = "moonshine" + (/^n(:[0-9a-f]{8})$/.exec(network) || [,""])[1];
      console.log(name);
      var openreq = indexedDB.open("moonshine", 1);
      $.merge(openreq, {
        onsuccess: function (evt) { return resolve(openreq.result) },
        onerror: function (evt) { console.error("openDb:", evt.target.error) },
        onupgradeneeded: function (evt) {
          ...
          if (network) {
            ...
            var user = store.createObjectStore("user", { keyPath: "id" })
          } else {
            var windows = store.createObjectStore("windows", {keyPath: "id"});
            var dbs = store.createObjectStore("dbs", {keyPath: "id"});
          }
        }
      })
    })
  ...
  dropdbs: function () {
    ...
    return $.initdb().then(function (db) {
      var dbstx = db.transaction("dbs", "readwrite"), dbs = dbstx.objectStore("dbs");
      wstx.oncomplete = function () { db.close() };
      return new Promise(function (resolve) {
        var dbs = [];
        ws.openCursor().onsuccess = function (e) {
          var cursor = e.target.result;
          if (cursor) { dbs.push(cursor.key.slice(1, 10)); cursor.continue() }
          else { ws.clear(), resolve(dbs) }
        };
      })
    }).then(function (dbs) {
      Promise.all( dbs.map(function (db) {
        return new Promise(function (resolve) { indexedDB.deleteDatabase("moonshine" + db).onsuccess = resolve })
      }) )
    })
  },
  exporting: {
  ...
  Models: function () {
    ...
    function user () {
      ...
      return $.merge(state(), {
        ...
        load_tmp: function (db) {
          var utx = db.transaction("user", "readwrite"), u = utx.objectStore("user")
          utx.oncomplete = function () { db.close() };
          return new Promise(function (resolve) {
            u.openCursor().onsuccess = function (e) {
              var cursor = e.target.result;
              if (cursor) models.user.id = cursor.key;
              else {
                self.user.id || u.add({
                  id: ( self.user.id = "u:" + $.hash( String.fromCharCode.apply(null, crypto.getRandomValues(new Uint8Array(100))) ) )
                });
                self.page.users = [self.user.public];
                return resolve()
              }
            }
          })
    ...
    function page () {
      var room = { items: [] }, rel = [
        ...
        return $.initdb(localStorage.network_id).then(function (db) {
      ...
      return $.merge(state(), {
        klass: "page",
        room: room,
        refresh: function (lastRefresh) { // TODO: user list refresh
          return getRoomStore(0).then(function (rs) {
            ...
            return new Promise(function (resolve) {
              rs.index("by_timestamp").openCursor(IDBKeyRange.lowerBound(lastRefresh || 0)).onsuccess = function (e) {
                ...
                } else if ("comments" in room.items[index]) rel[0].items.forEach(function (k, i) {
                  !i || (index = diff.items.map(function (a) { return a.id }).indexOf(id)) === -1 || delete diff.items[index][k]
                });
    ...
    var ts = Date.now(), id = "w:" + $.hash( String.fromCharCode.apply(null, crypto.getRandomValues(new Uint8Array(100))) ), self;
    return self = { //TODO: group everything into MVC,
      user: user(),
      page: page(),
      txpool: txpool(),
      info: {id: id, timestamp: ts}
    }
  },
  ...
  peersprune: function (peers) {
    return peers.filter(function (p) {
      ...
      p.dataChannels.length || (models.page.users = models.page.users.filter(function (u) { return u.id !== p.user_id }));
...
function init() {
  ...
  $.addEvents({
    ...
    "": {
      ...
      storage: function (e, v) {
        if (!e.key) {
          $.initdb().then(function (db) {
            var wstx = db.transaction("windows", "readwrite"), ws = wstx.objectStore("windows");
            wstx.oncomplete = function () { db.close() };
            return new Promise(function (resolve) {
              ws.openCursor().onsuccess = function (e) {
                var cursor = e.target.result, wl = [];
                if (cursor) { wl.push(cursor.key); cursor.continue() }
                else {
                  var info = { id: models.info.id, timestamp: models.info.timestamp };
                  ws.add(info); wl.push(info); return resolve(wl)
                }
              };
            })
          }).then(function (wl) {
            var ws = JSON.parse(localStorage.windows || "[]");
            if (ws.length < wl.length) localStorage.windows = JSON.stringify(wl);
            localStorage.timestamp = wl.reduce(function (p, c) { return Math.min(p.timestamp, c.timestamp) }).timestamp;
            if (peers.length) localStorage.online = true;
            localStorage.windowCount++
          })
        } else if (v = localStorage.newRTCOffer) {
          delete localStorage.newRTCOffer;
          if (models.info.timestamp != localStorage.timestamp) return;
        ...
        } else if (models.info.timestamp != localStorage.timestamp && (v = localStorage.newRTCAnswer)) {
          ...
          if (JSON.parse(localStorage.user).id && models.info.pagestate === "ready" && e.key === "online") {
            models.user.refresh();
            return models.page.refresh().then(openRoom);
          }
  ...
  function refreshSession () {
    $.initdb().then(function (db) {
      var
        wstx = db.transaction("windows", "readwrite"), ws = wstx.objectStore("windows"),
        info = { id: models.info.id, timestamp: models.info.timestamp };
      wstx.oncomplete = function () { db.close() };
      ws.clear();
      ws.add(info);
      localStorage.clear();
      localStorage.timestamp = localStorage.timestamp || models.info.timestamp;
      localStorage.windows = JSON.stringify([info]);
      localStorage.windowCount = 1;
  ...
  function openChannel () {
    localStorage.network_id || this.send( JSON.stringify({ network: this.pc.ondatachannel === null ? "connect" : "create" }) );
    if (peers.length === 1) {
      this.pc.ondatachannel === null || requestConn("new"); //TODO: user-defined maxPeers
    ...
    //!models.page.room.length || dc.send( JSON.stringify({ update: models.page.room }) )
  }
  
  function openRoom (room) {
    updatePage(room);
    models.info.pagestate = "ready";
    if (localStorage.windowCount || localStorage.online) {
      enterRoom();
      $.scrollPanel($.scrolling.panel = 1, "linear", 0);
      models.info.pagestate = "connecting";
      requestConn("existing")
    } else if (!localStorage.online) delete localStorage.newRTCOffer
  }
  ...
  function closeChannel (e) {
    ...
    !peers.length || closeConn([].concat.apply( [], peers.map(function (p) { return p.dataChannels }) ));
    models.user.clear();
    if (e && e.type === "beforeunload") models.info.pagestate = "closed";
    else initModels();
  }
  ...
  function closeConn (dcs) {
    ...
    else if (peers.length === 0) { closeRoom(); initModels() }
  }
  ...
  function closeRoom () {
    ...
    --localStorage.windowCount || delete localStorage.online
  }
  ...
  function getMessage (e) {
    ...
    console.log("get " + this.pc.peer.user_id + " msg:", e.data);
    if ("network" in data) {
      if (data.network === "connect") {
        if ("network_id" in localStorage) this.send( JSON.stringify({ network: localStorage.network_id }) )
      } else if (data.network === "create") {
        var hash = $.hash( String.fromCharCode.apply(null, crypto.getRandomValues(new Uint8Array(100))) );
        this.send( JSON.stringify({ network: ( localStorage.network_id = models.network.id = "n:" + hash ) } ))
      } else {
        localStorage.network_id = models.network.id = data.network
      }
      if (peers.length === 1) this.send( JSON.stringify({ sharestate: { self: models.user.public }, action: "open" }) );
      else if (peers.length > 1) this.send( JSON.stringify({ sharestate: { self: models.user.public, users: models.page.users }, action: "open" }) );
      $.initdb(localStorage.network_id).then(models.user.load_tmp).then(models.page.refresh).then(models.txpool.refresh).then(openRoom)
        .then(function () { !models.page.room.length || this.send( JSON.stringify({ update: models.page.room }) ) }.bind(this))
      ...
      if (data.action === "open") {
        ...
        if ("self" in data.sharestate) {
          if (peers.map(function (p) {return p.user_id}).indexOf(data.sharestate.self.id) !== -1 &&
            models.info.timestamp == localStorage.timestamp) return closeConn([this]);
          else this.pc.peer.user_id = data.sharestate.self.id; // can be spoofed
        }
  ...
  function sendMessage () {
    ...
    if (this.parentNode.parentNode.id === "room" && messageBody.value) {
      ...
      messageTitle.value = messageBody.value = "";
      toggleNewItem()
    } else if (this.parentNode.parentNode.className === "page-item" && (ta = $(".message", this.parentNode)).value) {
  ...
  function updatePage (payload, vm) {
    console.log("update page:", JSON.stringify(payload))
    if ((vm = $.validate(payload)) !== "valid") {
      var bad = JSON.stringify(payload);
      console.log("bad message " + bad.length + " bytes: " + vm);
      if (bad.length < 65536) console.log(bad);
      return
    }
    if ("items" in payload && payload.items.length) {
      for (var i = 0, iE, c, j; i < payload.items.length; i++) {
        iE =
          $("#" + payload.items[i].id) ||
          (function (iE_) {
            $.addAttr(iE_, {id: payload.items[i].id, timestamp: payload.items[i].timestamp});
            $(".title", iE_).innerHTML = $.markdown(payload.items[i].title, "title");
            $(".body", iE_).innerHTML = $.markdown(payload.items[i].body, "body");
            $(".user-id", iE_).innerHTML = payload.items[i].user_data.id;
            $.addAttr($(".user-id", iE_), { "data-user": payload.items[i].user_data.id });
            models.page.users.map(function (u) { return u.id }).indexOf(payload.items[i].user_data.id) + 1 ||
              $(".user-id", iE_).classList.add("abandoned");
            return iE_
          })($("#stamps > .page-item").cloneNode(true));
        if ("comments" in payload.items[i]) {
          if (!$(".comments > *", iE) || $(".form.hide"), iE) $(".comments", iE).className = "comments";
          for (c = 0; c < payload.items[i].comments.length; c++) {
            $.insert($(".comments", iE), (function (cE_) {
              $.addAttr(cE_, {id: payload.items[i].comments[c].id, timestamp: payload.items[i].comments[c].timestamp});
              $(".body", cE_).innerHTML = $.markdown(payload.items[i].comments[c].body, "body");
              $(".user-id", cE_).innerHTML = payload.items[i].comments[c].user_data.id;
              $.addAttr($(".user-id", cE_), { "data-user": payload.items[i].comments[c].user_data.id });
              models.page.users.map(function (u) { return u.id }).indexOf(payload.items[i].comments[c].user_data.id) + 1 ||
                $(".user-id", cE_).classList.add("abandoned");
              return cE_
            })($("#stamps > .item-comment").cloneNode(true)), function (a) {return a.getAttribute("timestamp")}, function (a, b) {return a < b});
            if ($(".form.hide", iE)) $(".item > .unread", iE).dataset.num++
          }
        }
        if (!$(".conversation > *")) $.toggleClasses({"#room": "active"});
        $.insert( itemsWindow, iE, function (a) { return ($(".comments > :last-child", a) || a).getAttribute("timestamp") }, function (a, b) {return a > b} );
        if ("timestamp" in payload.items[i]) {
          $(".message", iE).setAttribute("maxlength", $.charlim.body);
          $.addEvent($(".send", iE), "click", sendMessage);
          $.addEvent($(".message", iE), "keyup", function (e) {
            if (e.keyCode === 13 && e.ctrlKey === true) sendMessage.bind(e.target)();
            return false
          });
          $.addEvent($(".toggle", iE), "click", function () {
            if (this.innerHTML === "[reply]") {
              ...
              $(".unread", this.parentNode).dataset.num = 0
      ...
      for (var i = 0, u; i < payload.users.length; i++) {
        if ( u = $("#" + payload.users[i].id.replace(/:/, "\\:")) ) userList.removeChild(u);
          ...
          $.insert( userList, (function (uE_) {
            ...
            $(".user-id", uE_).innerHTML = payload.users[i].id;
            return uE_
          })($("#stamps > .connected-user").cloneNode(true)), function (a) { return a.getAttribute("id") }, function (a, b) {return a < b} );
        }
        models.info.pagestate === "initialising" || $.toggleClasses([[".conversation .user-id[data-user='" + payload.users[i].id + "']", "abandoned"]]);
      }
    }
  }
